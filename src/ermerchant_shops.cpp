...
static constexpr uint8_t lot_item_category_goods = 1;

static constexpr uint8_t cost_type_lost_ashes_of_war = 4;

static constexpr uint8_t equip_type_weapon = 0;
static constexpr uint8_t equip_type_protector = 1;
static constexpr uint8_t equip_type_accessory = 2;
static constexpr uint8_t equip_type_goods = 3;
static constexpr uint8_t equip_type_gem = 4;

static constexpr uint8_t weapon_type_arrow = 81;
static constexpr uint8_t weapon_type_greatarrow = 83;
static constexpr uint8_t weapon_type_bolt = 85;
static constexpr uint8_t weapon_type_ballista_bolt = 86;

static constexpr int64_t weapon_unarmed_id = 110000;
...
static constexpr int64_t protector_bare_head_id = 10000;
static constexpr int64_t protector_bare_chest_id = 10100;
static constexpr int64_t protector_bare_arms_id = 10200;
static constexpr int64_t protector_bare_legs_id = 10300;
...
static constexpr uint8_t goods_type_crafting_material = 2;
static constexpr uint8_t goods_type_remembrance = 3;
static constexpr uint8_t goods_type_sorcery = 5;
static constexpr uint8_t goods_type_spirit_summon_lesser = 7;
static constexpr uint8_t goods_type_spirit_summon_greater = 8;
static constexpr uint8_t goods_type_wondrous_physick = 9;
static constexpr uint8_t goods_type_wondrous_physick_tear = 10;
static constexpr uint8_t goods_type_regenerative_material = 11;
static constexpr uint8_t goods_type_info_item = 12;
static constexpr uint8_t goods_type_reinforcement_material = 14;
static constexpr uint8_t goods_type_great_rune = 15;
static constexpr uint8_t goods_type_incantation = 16;
static constexpr uint8_t goods_type_self_buff_sorcery = 17;
static constexpr uint8_t goods_type_self_buff_incantation = 18;
// Convergence has a unique goods type for consumable runes and remembrances
static constexpr uint8_t goods_type_convergence_rune = 100;
static constexpr uint8_t goods_type_convergence_remembrance = 101;

static constexpr uint8_t goods_sort_group_tutorial = 20;
static constexpr uint8_t goods_sort_group_gesture = 250;

/*
 * Items that are automatically given to the player in certain scenarios, and should not be
 * purchasable
 */
static const std::set<int64_t> excluded_goods = {
...
static const std::set<int64_t> cut_content_goods = {
    8860,    // Erdtree Prayerbook
    8861,    // Erdtree Codex
    2008023, // Keep Wall Key
};

static const std::set<int64_t> cut_content_protectors = {
    610000, // Ragged Hat
    610100, // Ragged Armor
...
static const std::set<int64_t> cut_content_weapons = {
    33290000, // [ERR] ST Staff
};

static const std::set<int64_t> cut_content_accessories = {
...
static std::map<int16_t, uint8_t> max_level_by_reinforce_type_id;

// Goods that shouldn't be allowed in the storage box, because acquiring a second copy can break
// things
static std::set<uint32_t> no_repository_item_ids;

static bool is_shop_open = false;

static shop *get_mod_shop(int32_t shop_lineup_id)
{
...
static from::find_shop_menu_result *(*solo_param_repository_lookup_shop_menu)(
    from::find_shop_menu_result *result, uint8_t shop_type, int32_t begin_id, int32_t end_id);
...
static void (*solo_param_repository_lookup_shop_lineup)(from::find_shop_menu_result *, uint8_t,
                                                        int32_t id);
...
static void (*open_regular_shop)(void *, int64_t, int64_t);

/**
 * Hook for OpenRegularShop()
 */
static void open_regular_shop_detour(void *unk, int64_t begin_id, int64_t end_id)
{
        ...
        (shop->id == ermerchant::shops::weapons || shop->id == ermerchant::shops::dlc_weapons))
    {
        ...
        for (auto &lineup : shop->lineups)
        {
            if (lineup.equipType == equip_type_weapon)
            {
                ...
                auto reinforce_level = (int32_t)std::floor(
                    (max_reinforce_level + 0.5) *
...
static int32_t (*get_sell_value)(uint32_t *);
 ...
 */
static int32_t get_sell_value_detour(uint32_t *item_id)
{
    if (is_shop_open)
    {
        return 0;
...
static uint64_t (*get_max_repository_num)(uint32_t *);
 ...
 */
static uint64_t get_max_repository_num_detour(uint32_t *item_id)
{
...
static uint32_t (*get_event_flag)(void *, uint32_t);
 ...
 */
static uint32_t get_event_flag_detour(void *self, uint32_t flag_id)
{
    switch (flag_id)
    {
...
void ermerchant::setup_shops()
{
    ...
    // as stock counters.
    std::map<uint32_t, int32_t> goods_flag_counts;
